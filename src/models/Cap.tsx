...
function Cap({ scroll }: { scroll: Scroll }) {
  ...
  const { position, rotation } = useLocation("right", [-0.1 * Math.PI, 0, 0.2 * Math.PI]);
  ...
  useFrame(({ clock }) => {
    ...
    const t = Math.min(Math.abs(scroll.progress - 0.4), 0.4);
    ref.current.rotation.x = rotation[0] + t * 10;
    ref.current.position.x = position[0] + t * 20;
  })

  return (
    <group dispose={null} ref={ref} position={position} rotation={rotation} scale={0.8}>
