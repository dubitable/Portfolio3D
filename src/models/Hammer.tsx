...
function Hammer({ scroll }: { scroll: Scroll }) {
  ...
  const { position, rotation, scale } = useLocation("left", [0, 0.2 * Math.PI, Math.PI * 0.1]);
  ...
  useFrame(({ clock }) => {
    ...
    const t = Math.min(Math.abs(scroll.progress - 0.75), 0.75);
  ...
  return (
    <>
      <group dispose={null} ref={ref} position={position} rotation={rotation} scale={2 * scale}>
